...
final class HeadlessUniversalCheckoutAutoTests: XCTestCase {
    ...
    func testNativeUIManager_presentAdyenGiroPay_withoutSurcharge_abort() throws {
        let apiConfiguration = setupApiConfiguration(paymentMethod: nativePaymentMethod)
        setupMockApiClients(apiConfiguration: apiConfiguration)

        let orderedExpectations = expectationsForDelegates(paymentMethod: nativePaymentMethod, shouldAbort: true)
        try presentNativeUIManager(paymentMethod: nativePaymentMethod, expecting: orderedExpectations)
    }

    // MARK: RawDataManager Tests

    func testRawDataManager_presentPaymentCard_withoutSurcharge() throws {
        let apiConfiguration = setupApiConfiguration(paymentMethod: rawDataPaymentMethod)
        setupMockApiClients(apiConfiguration: apiConfiguration)
    ...
    func testRawDataManager_paymentCardPayment_withoutSurcharge_abort() throws {
        let apiConfiguration = setupApiConfiguration(paymentMethod: rawDataPaymentMethod)
        setupMockApiClients(apiConfiguration: apiConfiguration)

        let orderedExpectations = expectationsForDelegates(
...
extension HeadlessUniversalCheckoutAutoTests {
