...
/// Tests for DefaultValidationService that orchestrates validation across different field types.
@available(iOS 15.0, *)
final class ValidationServiceTests: XCTestCase {
    ...
    func test_validateCardNumber_callsRulesFactory() {
        // Given
        let cardNumber = TestData.CardNumbers.validVisa

        // When
        _ = sut.validateCardNumber(cardNumber)

        // Then
        XCTAssertEqual(mockRulesFactory.createCardNumberRuleCallCount, 1)
    }
    ...
    func test_validateExpiry_callsRulesFactory() {
        // Given
        let expiry = TestData.ExpiryDates.validFuture

        // When
        _ = sut.validateExpiry(month: expiry.month, year: expiry.year)

        // Then
        XCTAssertEqual(mockRulesFactory.createExpiryDateRuleCallCount, 1)
    }
    ...
    func test_validateCVV_callsRulesFactoryWithNetwork() {
        // Given
        let cvv = TestData.CVV.valid3Digit
        let network = CardNetwork.visa

        // When
        _ = sut.validateCVV(cvv, cardNetwork: network)

        // Then
        XCTAssertEqual(mockRulesFactory.createCVVRuleCallCount, 1)
        XCTAssertEqual(mockRulesFactory.lastCardNetwork, network)
    }
    ...
    func test_validateCardholderName_callsRulesFactory() {
        // Given
        let name = TestData.CardholderNames.valid

        // When
        _ = sut.validateCardholderName(name)

        // Then
        XCTAssertEqual(mockRulesFactory.createCardholderNameRuleCallCount, 1)
    }
