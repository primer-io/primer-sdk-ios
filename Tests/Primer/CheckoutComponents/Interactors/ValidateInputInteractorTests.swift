...
/// Tests for ValidateInputInteractorImpl that validates individual and multiple input fields.
@available(iOS 15.0, *)
final class ValidateInputInteractorTests: XCTestCase {
    ...
    func test_validate_withInvalidInput_returnsInvalidResult() async {
        ...
        XCTAssertEqual(result.errorCode, "invalid-card")
    ...
    func test_validateMultiple_withMixedResults_returnsCorrectResults() async {
        ...
        mockValidationService.stubResult(
            for: .cvv,
            result: ValidationResult.invalid(code: "invalid-cvv", message: "CVV is invalid")
        )
        ...
        XCTAssertEqual(results[.cvv]?.errorCode, "invalid-cvv")
    }
