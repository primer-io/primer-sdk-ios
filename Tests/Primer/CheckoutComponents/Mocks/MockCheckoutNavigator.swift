...
final class MockCheckoutNavigator: ObservableObject {
    ...
    /// Emits a route through the navigation events stream
    func emit(_ route: CheckoutRoute) {
        emittedRoutes.append(route)
        continuation?.yield(route)
    }

    /// Finishes the navigation events stream
    func finish() {
        continuation?.finish()
    }
    ...
    /// Total number of navigation calls
...
extension MockCheckoutNavigator {
    ...
    /// Creates a navigator pre-configured at payment method selection
    static func atPaymentSelection() -> MockCheckoutNavigator {
        let navigator = MockCheckoutNavigator(coordinator: .atPaymentSelection())
        return navigator
    }

    /// Creates a navigator pre-configured at card payment screen
    static func atCardPayment() -> MockCheckoutNavigator {
        let navigator = MockCheckoutNavigator(coordinator: .atCardPayment())
        return navigator
    }

    /// Creates a navigator with underlying coordinator that blocks navigation
    static func withNavigationBlocked() -> MockCheckoutNavigator {
        let navigator = MockCheckoutNavigator(coordinator: .withNavigationBlocked())
        return navigator
    }
}
