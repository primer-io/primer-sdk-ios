...
final class MockCheckoutCoordinator: ObservableObject {
    ...
    /// Sets up navigation stack with initial routes for testing
    func setupNavigationStack(_ routes: [CheckoutRoute]) {
        navigationStack = routes
    }

    /// Verifies that a specific route was navigated to
    func didNavigate(to route: CheckoutRoute) -> Bool {
        allNavigatedRoutes.contains(route)
    }

    /// Verifies navigation sequence matches expected order
...
extension MockCheckoutCoordinator {
    ...
    /// Creates a coordinator pre-configured at payment method selection
    ...
    /// Creates a coordinator pre-configured with a card payment method screen
    static func atCardPayment() -> MockCheckoutCoordinator {
        let coordinator = MockCheckoutCoordinator()
        coordinator.navigationStack = [.paymentMethodSelection, .paymentMethod("PAYMENT_CARD", .fromPaymentSelection)]
        return coordinator
    }

    /// Creates a coordinator with navigation blocking enabled
