    ...
    func test_announcements_eachType_usesCorrectNotificationType() {
        ...
        let testCases: [(
            ...
            (
                { $0.announceError($1) },
                .announcement,
                TestData.Accessibility.errorMessage,
                TestData.Accessibility.errorDescription
            ),
            (
                { $0.announceStateChange($1) },
                .announcement,
                TestData.Accessibility.stateChangeMessage,
                TestData.Accessibility.stateChangeDescription
            ),
            (
                { $0.announceLayoutChange($1) },
                .layoutChanged,
                TestData.Accessibility.layoutChangeMessage,
                TestData.Accessibility.layoutChangeDescription
            ),
            (
                { $0.announceScreenChange($1) },
                .screenChanged,
                TestData.Accessibility.screenChangeMessage,
                TestData.Accessibility.screenChangeDescription
            )
        ]
