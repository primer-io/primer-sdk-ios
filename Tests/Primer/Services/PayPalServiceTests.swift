...
class PayPalServiceTests: XCTestCase {
    ...
    func test_startOrderSession_fails_if_client_token_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal payment sesion | Failure: No client token")
        ...
        Task {
            do {
                _ = try await service.startOrderSession()
                XCTFail("Test should not get into the success case.")
            } catch {
                // Expected error
            }
...
//        let api = MockPrimerAPIClient(with: data, throwsError: false)
    ...
    func test_startOrderSession_fails_if_configId_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal payment sesion | Failure: No config ID")
    ...
    func test_startBillingAgreementSession_fails_if_client_token_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal billing agreement | Failure: No client token")
...
//        let api = MockPrimerAPIClient(with: data, throwsError: false)
    ...
    func test_startBillingAgreementSession_fails_if_configId_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal billing agreement | Failure: No config ID")
...
//        let api = MockPrimerAPIClient(with: data, throwsError: false)
    ...
    func test_confirmBillingAgreement_fails_if_client_token_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal billing agreement | Failure: No client token")
    ...
    func test_confirmBillingAgreement_fails_if_configId_nil_async() throws {
        let expectation = self.expectation(description: "Create PayPal billing agreement | Failure: No config ID")
    ...
    func testStartOrderSession() throws {
        ...
        apiClient.onCreateOrderSession = { _, _ in
            .init(orderId: "order_id", approvalUrl: "scheme://approve")
        }
    ...
    func testStartOrderSession_async() throws {
        let apiClient = MockPayPalAPIClient()
        let service = PayPalService(apiClient: apiClient)
        ...
        Task {
            do {
                let model = try await service.startOrderSession()
                XCTAssertEqual(model.orderId, "order_id")
                XCTAssertEqual(model.approvalUrl, "scheme://approve")
            } catch {
                XCTFail()
    ...
    func testCreateBillingAgreement() throws {
        ...
        apiClient.onCreateBillingAgreementSession = { _, _ in
            .init(tokenId: "my_token", approvalUrl: "scheme://approve")
        }
    ...
    func testCreateBillingAgreement_async() throws {
        let apiClient = MockPayPalAPIClient()
        let service = PayPalService(apiClient: apiClient)
        ...
        Task {
            do {
                let approvalUrl = try await service.startBillingAgreementSession()
                XCTAssertEqual(approvalUrl, "scheme://approve")
            } catch {
                XCTFail()
            }
...
private class MockPayPalAPIClient: PrimerAPIClientPayPalProtocol {
    ...
    var onCreateBillingAgreementSession: ((DecodedJWTToken, Request.Body.PayPal.CreateBillingAgreement) -> Response.Body.PayPal
