...
"""
        ...
        do {
            ...
            service.createPayment(paymentRequest: Payment.CreateRequest(paymentMethodToken: "a_payment_method_token")) { response, error in
...
"""
        ...
        do {
            ...
            let service = CreateResumePaymentService()
            service.resumePaymentWithPaymentId("AUOhM7E6", paymentResumeRequest: Payment.ResumeRequest(resumeToken: "a_resume_token")) { response, error in
                guard let paymentResponse = response else {
                          if let error = error {
                              XCTAssert(false, error.localizedDescription)
                          }
