...
class PrimerRawDataManagerTests: XCTestCase {
    ...
    func test_list_primer_payment_method_types() throws {
        ...
        var paymentMethodTypes: [PrimerPaymentMethodType]?
        ...
        if !(paymentMethodTypes ?? []).contains(.paymentCard) ||
            !(paymentMethodTypes ?? []).contains(.applePay) ||
            !(paymentMethodTypes ?? []).contains(.adyenGiropay)
        {
    ...
    func test_list_primer_input_elements() throws {
        ...
        let inputElementTypes = PrimerHeadlessUniversalCheckout.current.listRequiredInputElementTypes(for: .paymentCard)
