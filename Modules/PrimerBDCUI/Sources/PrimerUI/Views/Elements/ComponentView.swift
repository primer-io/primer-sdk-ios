...
struct ComponentView<C: View>: View {
    ...
    private var errorMessage: String? { componentArgs.fieldID.flatMap(viewModel.errorMessage) }
...
private extension ComponentView {
    ...
    func makeTextFieldView(props: TextFieldProps) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            if !props.value.isEmpty, let errorMessage {
                Text(errorMessage).foregroundColor(.red).font(.caption).padding(.bottom, 2)
            }
            HStack {
