...
internal class PMKFinalizer {
    let pending = Guarantee<Void>.pending()

    /// `finally` is the same as `ensure`, but it is not chainable
    internal func finally(on: Dispatcher = conf.D.return, _ body: @escaping () -> Void) {
...
internal class PMKCascadingFinalizer {
     ...
     */
    @discardableResult
    internal func `catch`(on: Dispatcher = conf.D.return, policy: CatchPolicy = conf.catchPolicy, _ body: @escaping(Error) -> Void) -> PMKFinalizer {
    ...
    /**
     ...
     - SeeAlso: [Cancellation](http://promisekit.org/docs/)
     */
    internal func `catch`<E: Swift.Error>(only: E, on: Dispatcher = conf.D.return, _ body: @escaping(E) -> Void) -> PMKCascadingFinalizer where E: Equatable {
    ...
    /**
     ...
     - SeeAlso: [Cancellation](http://promisekit.org/docs/)
     */
    internal func `catch`<E: Swift.Error>(only: E.Type, on: Dispatcher = conf.D.return, policy: CatchPolicy = conf.catchPolicy, _ body: @escaping(E) -> Void) -> PMKCascadingFinalizer {
     ...
     */
    @discardableResult
    internal func cauterize() -> PMKFinalizer {
        return self.catch {
