...
final class ManifestRepository {
    ...
    func fetchManifest(publicKeyB64s: [String]) async throws -> Manifest {
        let signedManifest: SignedManifest = try await request(.manifest)
        guard ManifestValidator.isValid(signedManifest, publicKeyB64s: publicKeyB64s) else { throw Error.invalidManifest }
