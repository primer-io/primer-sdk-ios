...
internal class URLSessionStack: NetworkService, LogReporter {
    ...
    func request<T: Decodable>(_ endpoint: Endpoint, completion: @escaping ResultCallback<T>) -> PrimerCancellable? {
        ...
        guard let url = url(for: endpoint) else {
            let err = InternalError.invalidUrl(url: "Base URL: \(endpoint.baseURL ?? "nil") | Endpoint: \(endpoint.path)", userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
        ...
        let dataTask = session.dataTask(with: request) { [logger] data, response, error in
            ...
            if let error = error {
                ...
                let err = InternalError.underlyingErrors(errors: [error], userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
            ...
            guard let data = data else {
                ...
                let err = InternalError.noData(userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                ErrorHandler.handle(error: err)
                DispatchQueue.main.async { completion(.failure(err)) }
                return
            }
            ...
            do {
                ...
                if let json = try? JSONSerialization.jsonObject(with: data, options: .allowFragments), let jsonDic = json as? [String: Any?],
                   ...
                   let statusCode = (response as? HTTPURLResponse)?.statusCode {
                    ...
                    if statusCode == 401 {
                        let err = InternalError.unauthorized(url: urlStr, method: endpoint.method, userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                        ErrorHandler.handle(error: err)
                        ...
                        let err = InternalError.serverError(status: statusCode, response: primerErrorResponse, userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                        ErrorHandler.handle(error: err)
                    ...
                    let err = InternalError.failedToDecode(message: "Failed to decode response from URL: \(urlStr)", userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                    ErrorHandler.handle(error: err)
                    ...
                    DispatchQueue.main.async { completion(.failure(InternalError.underlyingErrors(errors: [err], userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString))) }
                }
