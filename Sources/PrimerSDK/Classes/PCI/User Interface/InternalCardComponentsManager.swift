        ...
        do {
            ...
            .done { _ in
                ...
                guard let tokenizationPaymentInstrument = self.tokenizationPaymentInstrument else {
                    let err = handled(primerError: .invalidValue(key: "Payment Instrument", value: self.tokenizationPaymentInstrument))
                ...
                .catch { err in
                    _ =  handled(primerError: .underlyingErrors(errors: [err]))
                    self.delegate.cardComponentsManager?(self, tokenizationFailedWith: [err])
                }
            }
            .catch { err in
