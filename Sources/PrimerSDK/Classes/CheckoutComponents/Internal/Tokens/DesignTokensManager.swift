...
final class DesignTokensManager: ObservableObject {
    ...
    func fetchTokens(for colorScheme: ColorScheme) async throws {
        ...
        let mergedDict = colorScheme == .dark
            ? mergeDictionaries(baseDict, with: try loadJSON(named: "dark"))
            : baseDict

        // Process tokens through transformation pipeline
        var processedDict = resolveReferences(in: mergedDict)
        processedDict = convertHexColors(in: processedDict)
        var flatDict = flattenTokenDictionary(processedDict)
        flatDict = resolveFlattenedReferences(in: flatDict, source: processedDict)
        flatDict = evaluateMath(in: flatDict)
    ...
    private func loadJSON(named fileName: String) throws -> [String: Any] {
        guard let url = Bundle.primerResources.url(forResource: fileName, withExtension: "json"),
            ...
            throw NSError(domain: "DesignTokensManager", code: 1,
                         userInfo: [NSLocalizedDescriptionKey: "Failed to load \(fileName).json"])
        }
        return dictionary
    }
