...
struct VaultSection: View {
        ...
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: PrimerRadius.medium(tokens: tokens))
                    .fill(isPayButtonEnabled ? CheckoutColors.borderFocus(tokens: tokens) : CheckoutColors.gray300(tokens: tokens))
            )
        }
    ...
    private var cardNetwork: CardNetwork {
        let network = vaultedPaymentMethod.paymentInstrumentData.network ??
                     vaultedPaymentMethod.paymentInstrumentData.binData?.network ?? "Card"
