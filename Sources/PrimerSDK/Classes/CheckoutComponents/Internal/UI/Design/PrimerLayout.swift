...
struct PrimerComponentHeight {
    /// Error message container height (15px)
    /// Fixed height prevents layout shifts when error appears/disappears
    static let errorMessage: CGFloat = 15
...
/// Centralized icon size values
struct PrimerIconSize {
    /// Card network logo height (24px)
    /// Height for card network icons (Visa, Mastercard, etc.)
    static let cardNetwork: CGFloat = 24

    /// Payment method icon height (24px)
    /// Height for payment method icons in selection list
    static let paymentMethod: CGFloat = 24
}
