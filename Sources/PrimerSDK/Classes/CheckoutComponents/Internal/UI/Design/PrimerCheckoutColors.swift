...
struct PrimerCheckoutColors {
    // MARK: - Private Fallback Colors

    /// Default primary text color (#212121)
    private static let defaultTextPrimary = Color(red: 0x21/255, green: 0x21/255, blue: 0x21/255)

    /// Default negative/error icon color (coral red)
    private static let defaultIconNegative = Color(red: 1.0, green: 0.45, blue: 0.47)

    // MARK: - Public Color Helpers

    /// Primary text color (for main content)
    static func textPrimary(tokens: DesignTokens?) -> Color {
        return tokens?.primerColorTextPrimary ?? defaultTextPrimary
    }
    ...
    static func iconNegative(tokens: DesignTokens?) -> Color {
        return tokens?.primerColorIconNegative ?? defaultIconNegative
    }

    /// Default border color
    static func borderDefault(tokens: DesignTokens?) -> Color {
        return tokens?.primerColorBorderOutlinedDefault ?? gray100(tokens: tokens)
    }
