    ...
    public var state: AsyncStream<StructuredCardFormState> {
        AsyncStream { continuation in
            continuation.yield(StructuredCardFormState(
                ...
                selectedNetwork: self.initialSelectedNetwork.map { AvailablePaymentMethodData(network: $0) },
                availableNetworks: self.initialAvailableNetworks.map { AvailablePaymentMethodData(network: $0) },
