...
/// Payment method available for selection.
public struct CheckoutPaymentMethod: Equatable, Identifiable {
    public let id: String
    /// e.g., "PAYMENT_CARD", "PAYPAL"
    public let type: String
    public let name: String
    public let icon: UIImage?
    public let metadata: [String: Any]?
    /// Minor currency units (e.g., 500 for $5.00).
    public let surcharge: Int?
    /// True when surcharge is unknown ("Fee may apply").
    public let hasUnknownSurcharge: Bool
    /// e.g., "+$5.00", "No additional fee"
