...
public class Primer {
    ...
    public func showPaymentMethod(_ paymentMethod: PaymentMethodConfigType, withIntent intent: PrimerSessionIntent, on viewController: UIViewController, with clientToken: String? = nil, completion: ((Error?) -> Void)? = nil) {
        ...
        case (.adyenPayshop, .checkout):
            flow = .checkoutWithAsyncPaymentMethod(paymentMethodType: .adyenPayshop)
        ...
        case (.twoCtwoP, .checkout):
            flow = .checkoutWithAsyncPaymentMethod(paymentMethodType: .twoCtwoP)

        case (.xfers, .checkout):
            flow = .checkoutWithAsyncPaymentMethod(paymentMethodType: .xfers)
        
        case (.opennode, .checkout):
            flow = .checkoutWithAsyncPaymentMethod(paymentMethodType: .opennode)
