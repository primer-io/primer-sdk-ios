...
public final class NolPayPaymentComponent: PrimerHeadlessCollectDataComponent {
    ...
    private func validateData(for data: NolPayPaymentCollectableData) {
        ...
        case .paymentData(cardNumber: let cardNumber,
            ...
            if cardNumber.isEmpty || !cardNumber.isNumeric {
                let error = handled(error: PrimerValidationError.invalidCardnumber(message: "Card number is not valid."))
                errors.append(error)
            }
    ...
    public func start() {
        ...
        #else
        let error = handled(primerError: .missingSDK(paymentMethodType: PrimerPaymentMethodType.nolPay.rawValue))
