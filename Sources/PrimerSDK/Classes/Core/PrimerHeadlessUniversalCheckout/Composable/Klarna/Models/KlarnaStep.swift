...
import UIKit
/**
 * This enum is used to communicate the result of attempting to create, authorize and finalize a payment session
 * with Klarna also handling the PaymentView.
 ...
 * - `paymentSessionFinalizationRequired`: Signals that the payment session requires finalization steps to be
 *  completed by the user or the system.
 *
 ...
 *
 * - `viewInitialized`: Indicates that the Klarna view has been initialized. This is the first step in the Klarna view
 *  handling process.
 * - `viewResized(height: CGFloat)`: Represents a change in the view's height, which may occur
 * when the Klarna view adjusts to display different content.
 *  The `height` parameter specifies the new height of the view.
 * - `viewLoaded`: Indicates that the Klarna view has finished loading its initial content and is ready for interaction.
 *  The `view` parameter represents the klarna view.
