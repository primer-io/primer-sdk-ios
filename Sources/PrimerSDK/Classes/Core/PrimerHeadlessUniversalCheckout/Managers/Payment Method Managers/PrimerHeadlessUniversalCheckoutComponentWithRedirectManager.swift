    ...
    @objc public final class ComponentWithRedirectManager: NSObject {
        ...
        public func provideBanksComponent(paymentMethodType: String) throws -> any PrimerHeadlessMainComponent {
            ...
            guard let tokenizationModel = try getTokenizationModel() else {
                let dictionary: Dictionary = .errorUserInfoDictionary(additionalInfo: ["message": "Unable to locate a correct payment method view model"])
                throw handled(primerError: .unsupportedPaymentMethod(paymentMethodType: paymentMethodType.rawValue, userInfo: dictionary))
            }
