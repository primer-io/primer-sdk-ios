    ...
    public final class CardFormUIManager: NSObject, PrimerInputElementDelegate {
        ...
        public func tokenize(withData data: PrimerHeadlessUniversalCheckoutInputData? = nil) {
            ...
            .then { paymentMethodTokenData -> Promise<PrimerCheckoutData?> in
                ...
                guard let tokenizationViewModel: PaymentMethodTokenizationViewModelProtocol = PrimerAPIConfiguration.paymentMethodConfigs?.filter({ $0.type == PrimerPaymentMethodType.paymentCard.rawValue }).first?.tokenizationViewModel else {
                    let err = PrimerError.missingPrimerConfiguration(userInfo: nil, diagnosticsId: nil)
                    throw err
                }
                
                let paymentModule: PaymentModuleProtocol = PaymentModule(paymentMethodTokenizationViewModel: tokenizationViewModel)
                return paymentModule.pay(with: paymentMethodTokenData)
            }
