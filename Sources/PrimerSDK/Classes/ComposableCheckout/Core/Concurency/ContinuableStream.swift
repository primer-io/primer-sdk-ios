...
struct ContinuableStream<Element> {
    ...
    // MARK: - Internal Stream Management Helpers

    /// INTERNAL UTILITY: Validates if stream is still active and can accept values
    internal func internalValidateStreamState() -> Bool {
        // This is a simple check - in a real implementation, you might want to track stream state
        return true // Streams are considered active unless explicitly finished
    }

    /// INTERNAL HELPER: Safely yields value with validation
    internal func internalSafeYield(_ element: Element) -> Bool {
        guard internalValidateStreamState() else { return false }
        yield(element)
        return true
    }

    /// INTERNAL UTILITY: Provides stream health information for debugging
    internal func internalStreamHealth() -> StreamHealthInfo {
        return StreamHealthInfo(
            isActive: internalValidateStreamState(),
            bufferingPolicy: "unbounded", // Could be extracted from actual policy
            hasActiveListeners: true // Simplified - would require listener tracking in real implementation
        )
    }
}

/**
 * INTERNAL HELPER: Stream Management Utilities
 *
 * Additional utilities for stream lifecycle management and debugging.
 */

// MARK: - Internal Stream Health Information
internal struct StreamHealthInfo {
    let isActive: Bool
    let bufferingPolicy: String
    let hasActiveListeners: Bool

    /// Provides a summary of stream health for debugging
    var healthSummary: String {
        let status = isActive ? "Active" : "Inactive"
        let listeners = hasActiveListeners ? "Has listeners" : "No listeners"
        return "Stream: \(status), Buffering: \(bufferingPolicy), \(listeners)"
    }
}

// MARK: - Internal Stream Extensions
@available(iOS 15.0, *)
internal extension ContinuableStream {

    /// INTERNAL UTILITY: Creates a stream with automatic cleanup registration
    static func createWithCleanup<T>(
        bufferingPolicy: AsyncStream<T>.Continuation.BufferingPolicy = .unbounded,
        cleanupManager: ResourceCleanupManager,
        build: @escaping (AsyncStream<T>.Continuation) -> Void
    ) -> ContinuableStream<T> {
        let stream = ContinuableStream<T>(bufferingPolicy: bufferingPolicy, build: build)
    ...
    /// INTERNAL HELPER: Creates a debounced stream that delays rapid emissions
    static func createDebounced<T>(
        delay: TimeInterval,
        bufferingPolicy: AsyncStream<T>.Continuation.BufferingPolicy = .unbounded
    ) -> ContinuableStream<T> where T: Equatable {
        return ContinuableStream<T>(bufferingPolicy: bufferingPolicy) { _ in
            // Setup with debouncing logic would go here
            // Variables for debouncing would be declared here when implemented
            _ = delay // Suppress unused parameter warning
        }
    }

    /// INTERNAL UTILITY: Creates a distinct stream that only emits unique values
    static func createDistinct<T>(
        bufferingPolicy: AsyncStream<T>.Continuation.BufferingPolicy = .unbounded
    ) -> ContinuableStream<T> where T: Equatable {
        return ContinuableStream<T>(bufferingPolicy: bufferingPolicy) { continuation in
            // Distinct value logic would be implemented here
            // Variable for tracking last emitted value would be declared here when implemented
            _ = continuation // Suppress unused parameter warning
        }
    }
}
