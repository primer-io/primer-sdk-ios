...
internal class OAuthViewModel: OAuthViewModelProtocol {
    ...
    private func generatePaypalPaymentInstrument(_ host: OAuthHost, with completion: @escaping (Error?) -> Void) -> PaymentMethodDetailsProtocol? {
        ...
        case .CHECKOUT:
            ...
            return PaymentMethodOptions.PayPal.Details(paypalOrderId: id)
        case .VAULT:
            guard let agreement = confirmedBillingAgreement else {
                generateBillingAgreementConfirmation(host, with: completion)
                return nil
            }
            return PaymentMethodOptions.PayPal.Details(paypalBillingAgreementId: agreement.billingAgreementId,
                                               shippingAddress: agreement.shippingAddress,
                                               externalPayerInfo: agreement.externalPayerInfo)
        }
    }

    func handleTokenization(request: PaymentMethodRequest, with completion: @escaping (Error?) -> Void) {
    ...
    func tokenize(_ host: OAuthHost, with completion: @escaping (Error?) -> Void) {
        if (host == .klarna) {
            ...
            if Primer.shared.flow.internalSessionFlow.vaulted {
                ...
                klarnaService.createKlarnaCustomerToken { [weak self] (result) in
                    ...
                    case .success(let response):
                        paymentMethodDetails = PaymentMethodOptions.Klarna.Details(klarnaAuthorizationToken: nil, klarnaCustomerToken: response.customerTokenId, sessionData: response.sessionData)
                ...
                klarnaService.finalizePaymentSession { [weak self] result in
                    ...
                    case .success(let res):
                        let state: AppStateProtocol = DependencyContainer.resolve()
                        paymentMethodDetails = PaymentMethodOptions.Klarna.Details(klarnaAuthorizationToken: state.authorizationToken, klarnaCustomerToken: nil, sessionData: res.sessionData)
            ...
            let request = PaymentMethodRequest(paymentMethodDetails: instrument, state: state)
