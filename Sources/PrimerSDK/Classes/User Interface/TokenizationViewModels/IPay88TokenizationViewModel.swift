...
final class IPay88TokenizationViewModel: PaymentMethodTokenizationViewModel {
    ...
    override func validate() throws {
        ...
        if self.config.id == nil {
            errors.append(handled(primerError: .invalidValue(key: "configuration.id")))
        }

        if (self.config.options as? MerchantOptions)?.merchantId == nil {
            errors.append(handled(primerError: .invalidValue(key: "configuration.merchantId")))
        }
        ...
        if (AppState.current.amount ?? 0) == 0 {
            errors.append(
                handled(primerError: .invalidClientSessionValue(
                    name: "amount",
        ...
        #if !canImport(PrimerIPay88MYSDK)
        errors.append(handled(primerError: .missingSDK(paymentMethodType: config.type, sdkName: "PrimerIPay88SDK")))
        #endif
