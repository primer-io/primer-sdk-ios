                ...
                self.triggerAsyncAction(self.transactionNo!) { (result: Result<Bool, Error>) in
                    ...
                    case .success:
                        ...
                        guard let redirectUrl = self.redirectUrl else {
                            return seal.reject(handled(primerError: .invalidUrl(url: self.redirectUrl?.absoluteString)))
                        }
    ...
    override func awaitUserInput() -> Promise<Void> {
        return Promise { seal in
            ...
            guard let statusUrl = self.statusUrl else {
                return seal.reject(handled(primerError: .invalidUrl(url: self.statusUrl?.absoluteString)))
            }
