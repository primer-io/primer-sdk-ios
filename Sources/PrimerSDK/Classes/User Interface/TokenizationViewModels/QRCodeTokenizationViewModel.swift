    ...
    private func evaluateFireDidReceiveAdditionalInfoEvent() -> Promise<Void> {
        return Promise { seal in
            ...
            guard isHeadlessDidReceiveAdditionalInfoImplemented else {
                let err = PrimerError.generic(message: "Delegate function 'primerHeadlessUniversalCheckoutDidReceiveAdditionalInfo(_ additionalInfo: PrimerCheckoutAdditionalInfo?)' hasn't been implemented. No events will be sent to your delegate instance.", userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                ErrorHandler.handle(error: err)
                seal.reject(err)
                return
            }
            ...
            case PrimerPaymentMethodType.rapydPromptPay.rawValue,
                ...
                guard let expiresAt = decodedJWTToken.expDate else {
                    let err = PrimerError.invalidValue(key: "decodedClientToken.expiresAt", value: decodedJWTToken.expiresAt, userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                    ErrorHandler.handle(error: err)
                    seal.reject(err)
                    return
                }
                ...
                guard let qrCodeString = decodedJWTToken.qrCode else {
                    let err = PrimerError.invalidValue(key: "decodedClientToken.qrCode", value: decodedJWTToken.qrCode, userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                    ErrorHandler.handle(error: err)
                    seal.reject(err)
                    return
                }
            ...
            if let additionalInfo = additionalInfo {
                ...
                let err = PrimerError.invalidValue(key: "additionalInfo", value: additionalInfo, userInfo: .errorUserInfoDictionary(), diagnosticsId: UUID().uuidString)
                ErrorHandler.handle(error: err)
                seal.reject(err)
            }
        }
