...
internal class BankSelectorViewController: PrimerFormViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.3, repeats: false) { _ in
            if self.tokenizationModule.tableView.superview == nil {
                let lastView = self.verticalStackView.arrangedSubviews.last!
                self.verticalStackView.removeArrangedSubview(lastView)
                self.verticalStackView.addArrangedSubview(self.tokenizationModule.tableView)
                self.tokenizationModule.tableView.translatesAutoresizingMaskIntoConstraints = false
            }
        }
    }
