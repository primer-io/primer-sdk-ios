...
extension DecodedJWTToken {
    ...
    func validate() throws {
        if accessToken == nil {
            let err = PrimerError.invalidClientToken(userInfo: .errorUserInfoDictionary(reason: "Access token is nil"),
        ...
        guard let expDate = expDate else {
            let err = PrimerError.invalidClientToken(userInfo: .errorUserInfoDictionary(reason: "Expiry date missing"),
                                                     diagnosticsId: UUID().uuidString)
            ErrorHandler.handle(error: err)
            throw err
        }
        ...
        if expDate < Date() {
            let err = PrimerError.invalidClientToken(userInfo: .errorUserInfoDictionary(reason: "Expiry datetime has passed."),
                                                     diagnosticsId: UUID().uuidString)
            ErrorHandler.handle(error: err)
            throw err
        }
