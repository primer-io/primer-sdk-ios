...
class VaultPaymentMethodViewController: UIViewController {
    ...
    var delegate: VaultPaymentMethodViewControllerDelegate?
...
extension VaultPaymentMethodViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = "**** **** **** \(viewModel.paymentMethods[indexPath.row].last4)"
...
//                deleteButton.setImage(icon, for: .normal)
            ...
            cell.accessoryType = viewModel.selectedId == viewModel.paymentMethods[indexPath.row].id ? .checkmark : .none
        }
        
        cell.separatorInset = .zero
        
        if (indexPath.row == viewModel.paymentMethods.count - 1) {
