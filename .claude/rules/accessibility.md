...
## Key Patterns
- **Identifiers**: `primer_checkout_components_{screen}_{component}_{element}` (snake_case). This is a cross-repo contract: the CC E2E page objects select on these strings. All identifiers live in the `AccessibilityIdentifiers` registry — never hardcode one in a view. Chrome rendered by shared components (the `Common` namespace: back, close, edit, done, delete, cancel, loading) omits the screen segment. Vaulted rows carry the vault token id: `primer_checkout_components_vaulted_payment_method_{id}_item`.
