//
//  ClientTokenServiceTests.swift
//  PrimerSDKTests
//
//  Created by Carl Eriksson on 03/01/2021.
//

import XCTest
@testable import PrimerSDK

class ClientTokenServiceTests: XCTestCase {
    
    func test_loadCheckoutConfig_calls_clientTokenRequestCallback() throws {
        let accessToken = "7651512a-d12e-46e1-b58c-d8c3afc8c8ee"
        let response = ClientTokenResponse(
            clientToken: "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..s08j_MIxosYNTfUNtLnJELxeLqRVryPvFtdZfEjie08",
            expirationDate: "2020-12-06T16:45:38.963044"
        )
        
        let settings = PrimerSettings(
            amount: 200,
            currency: .EUR,
            clientTokenRequestCallback: { completion in
                completion(.success(response))
            },
            onTokenizeSuccess: { (result, callback) in
            },
            theme: PrimerTheme(),
            uxMode: .CHECKOUT,
            applePayEnabled: false,
            customerId: "cid",
            merchantIdentifier: "mid",
            countryCode: .FR
        )
        
        let paymentMethodConfigService = MockPaymentMethodConfigService()
        
        let service = ClientTokenService(with: settings, and: paymentMethodConfigService)
        
        service.loadCheckoutConfig(with: { error in })
        
        XCTAssertEqual(paymentMethodConfigService.fetchConfigCalled, true)
        XCTAssertEqual(service.decodedClientToken?.accessToken, accessToken)
    }
    
}
